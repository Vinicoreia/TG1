                         ...
                         unsigned char **out, size_t *outsize)
{
    ...
    for (i = 0; i < lld_total; i++)
    {
        ...
        if (symbol == 0 && count >= 3)
        {
            if (use_18)
            {
                while (count >= 11)
                {
                    ...
                    if (!size_only)
                    {
                        ZIPFLI_APPEND_DATA(18, &rle, &rle_size);
                        ZIPFLI_APPEND_DATA(count2 - 11, &rle_bits, &rle_bits_size);
                    }
                    clcounts[18]++;
                    count -= count2;
                }
            ...
            if (use_17)
            {
                while (count >= 3)
                {
                    ...
                    if (!size_only)
                    {
                        ZIPFLI_APPEND_DATA(17, &rle, &rle_size);
                        ZIPFLI_APPEND_DATA(count2 - 3, &rle_bits, &rle_bits_size);
                    }
                    clcounts[17]++;
                    count -= count2;
                }
        ...
        if (use_16 && count >= 4)
        {
            ...
            if (!size_only)
            {
                ZIPFLI_APPEND_DATA(symbol, &rle, &rle_size);
                ZIPFLI_APPEND_DATA(0, &rle_bits, &rle_bits_size);
            }
            while (count >= 3)
            {
                ...
                if (!size_only)
                {
                    ZIPFLI_APPEND_DATA(16, &rle, &rle_size);
                    ZIPFLI_APPEND_DATA(count2 - 3, &rle_bits, &rle_bits_size);
                }
                clcounts[16]++;
                count -= count2;
    ...
    ZipfliCalculateBitLengths(clcounts, 19, 7, clcl);
    if (!size_only)
        ZipfliLengthsToSymbols(clcl, 19, 7, clsymbols);
...
*/
static void AddLZ77Data(const ZipfliLZ77Store *lz77,
                        ...
                        unsigned char **out, size_t *outsize)
{
    ...
    for (i = lstart; i < lend; i++)
    {
        ...
        {
            unsigned lls = ZipfliGetLengthSymbol(litlen);
            unsigned ds = ZipfliGetDistSymbol(dist);
            assert(litlen >= 3 && litlen <= 288);
            assert(ll_lengths[lls] > 0);
            assert(d_lengths[ds] > 0);
            AddHuffmanBits(ll_symbols[lls], ll_lengths[lls], bp, out, outsize);
            AddBits(ZipfliGetLengthExtraBitsValue(litlen),
                    ZipfliGetLengthExtraBits(litlen),
                    bp, out, outsize);
            AddHuffmanBits(d_symbols[ds], d_lengths[ds], bp, out, outsize);
            AddBits(ZipfliGetDistExtraBitsValue(dist),
                    ZipfliGetDistExtraBits(dist),
                    bp, out, outsize);
            testlength += litlen;
        }
    }
                                            ...
                                            size_t lstart, size_t lend)
{
    ...
    for (i = lstart; i < lend; i++)
    {
        ...
        {
            int ll_symbol = ZipfliGetLengthSymbol(lz77->litlens[i]);
            int d_symbol = ZipfliGetDistSymbol(lz77->dists[i]);
            result += ll_lengths[ll_symbol];
            result += d_lengths[d_symbol];
            result += ZipfliGetLengthSymbolExtraBits(ll_symbol);
            result += ZipfliGetDistSymbolExtraBits(d_symbol);
        }
    }
                                                  ...
                                                  size_t lstart, size_t lend)
{
    ...
    else
    {
        ...
        {
            result += ll_lengths[i] * ll_counts[i];
            result += ZipfliGetLengthSymbolExtraBits(i) * ll_counts[i];
        }
        for (i = 0; i < 30; i++)
        {
            result += d_lengths[i] * d_counts[i];
            result += ZipfliGetDistSymbolExtraBits(i) * d_counts[i];
        }
        result += ll_lengths[256]; /*end symbol*/
        return result;
    }
                                       ...
                                       size_t lstart, size_t lend)
{
    ...
    {
        size_t ll_counts[ZIPFLI_NUM_LL];
        size_t d_counts[ZIPFLI_NUM_D];
        ZipfliLZ77GetHistogram(lz77, lstart, lend, ll_counts, d_counts);
...
static double TryOptimizeHuffmanForRle(
    const ZipfliLZ77Store *lz77, size_t lstart, size_t lend,
    const size_t *ll_counts, const size_t *d_counts,
    unsigned *ll_lengths, unsigned *d_lengths)
{
    size_t ll_counts2[ZIPFLI_NUM_LL];
    size_t d_counts2[ZIPFLI_NUM_D];
    unsigned ll_lengths2[ZIPFLI_NUM_LL];
    unsigned d_lengths2[ZIPFLI_NUM_D];
    double treesize;
    double datasize;
    double treesize2;
    double datasize2;
    ...
    OptimizeHuffmanForRle(ZIPFLI_NUM_LL, ll_counts2);
    OptimizeHuffmanForRle(ZIPFLI_NUM_D, d_counts2);
    ZipfliCalculateBitLengths(ll_counts2, ZIPFLI_NUM_LL, 15, ll_lengths2);
    ZipfliCalculateBitLengths(d_counts2, ZIPFLI_NUM_D, 15, d_lengths2);
...
3-bit block header.
*/
static double GetDynamicLengths(const ZipfliLZ77Store *lz77,
                                size_t lstart, size_t lend,
                                unsigned *ll_lengths, unsigned *d_lengths)
{
    size_t ll_counts[ZIPFLI_NUM_LL];
    ...
    ZipfliCalculateBitLengths(ll_counts, ZIPFLI_NUM_LL, 15, ll_lengths);
    ZipfliCalculateBitLengths(d_counts, ZIPFLI_NUM_D, 15, d_lengths);
...
double ZipfliCalculateBlockSizeAutoType(const ZipfliLZ77Store *lz77,
                                        size_t lstart, size_t lend)
{
    double uncompressedcost = ZipfliCalculateBlockSize(lz77, lstart, lend, 0);
    /* Don't do the expensive fixed cost calculation for larger blocks that are
     unlikely to use it. */
    double fixedcost = (lz77->size > 1000) ? uncompressedcost : ZipfliCalculateBlockSize(lz77, lstart, lend, 1);
    double dyncost = ZipfliCalculateBlockSize(lz77, lstart, lend, 2);
...
*/
static void AddLZ77Block(const ZipfliOptions *options, int btype, int final,
                         const ZipfliLZ77Store *lz77,
                         size_t lstart, size_t lend,
                         size_t expected_data_size,
                         unsigned char *bp,
                         unsigned char **out, size_t *outsize)
{
    unsigned ll_lengths[ZIPFLI_NUM_LL];
    unsigned d_lengths[ZIPFLI_NUM_D];
    unsigned ll_symbols[ZIPFLI_NUM_LL];
    unsigned d_symbols[ZIPFLI_NUM_D];
...
{
    double uncompressedcost = ZipfliCalculateBlockSize(lz77, lstart, lend, 0);
    double fixedcost = ZipfliCalculateBlockSize(lz77, lstart, lend, 1);
    double dyncost = ZipfliCalculateBlockSize(lz77, lstart, lend, 2);
  ...
  blocks which already are pretty good with fixed huffman tree. */
    ...
    ZipfliInitLZ77Store(lz77->data, &fixedstore);
    if (expensivefixed)
    {
        /* Recalculate the LZ77 with ZipfliLZ77OptimalFixed */
        size_t instart = lz77->pos[lstart];
        size_t inend = instart + ZipfliLZ77GetByteRange(lz77, lstart, lend);

        ZipfliBlockState s;
        ZipfliInitBlockState(options, instart, inend, 1, &s);
        ZipfliLZ77OptimalFixed(&s, lz77->data, instart, inend, &fixedstore);
        fixedcost = ZipfliCalculateBlockSize(&fixedstore, 0, fixedstore.size, 1);
    ...
    ZipfliCleanLZ77Store(&fixedstore);
}

/*
Deflate a part, to allow ZipfliDeflate() to use multiple master blocks if
needed.
                       ...
                       size_t *outsize)
{
    ...
    {
        ZipfliBlockSplit(options, in, instart, inend,
                         options->blocksplittingmax,
                         &splitpoints_uncompressed, &npoints);
        splitpoints = (size_t *)malloc(sizeof(*splitpoints) * npoints);
    }

    ZipfliInitLZ77Store(in, &lz77);
    ...
    for (i = 0; i <= npoints; i++)
    {
        ...
        ZipfliInitBlockState(options, start, end, 1, &s);
        ZipfliLZ77Optimal(&s, in, start, end, options->numiterations, &store);
        totalcost += ZipfliCalculateBlockSizeAutoType(&store, 0, store.size);

        ZipfliAppendLZ77Store(&store, &lz77);
        if (i < npoints)
    ...
    if (options->blocksplitting && npoints > 1)
    {
        ...
        ZipfliBlockSplitLZ77(options, &lz77,
        ...
        for (i = 0; i <= npoints2; i++)
        {
            ...
            totalcost2 += ZipfliCalculateBlockSizeAutoType(&lz77, start, end);
        }
...
void ZipfliDeflate(const ZipfliOptions *options, int btype, int final,
    ...
    // TODO: depois ver como funciona a questão de smart blocks
    ZipfliDeflatePart(options, btype, final, in, 0, insize, bp, out, outsize);
}
