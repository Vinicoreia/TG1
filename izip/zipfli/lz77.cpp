...
void ZipfliStoreLitLenDist(unsigned short length, unsigned short dist,
  ...
  if (dist == 0) {
    ...
    store->size = origsize;
    ZOPFLI_APPEND_DATA(ZipfliGetLengthSymbol(length),
                       &store->ll_symbol, &store->size);
    store->size = origsize;
    ZOPFLI_APPEND_DATA(ZipfliGetDistSymbol(dist),
...
void ZipfliLZ77Greedy(ZipfliBlockState* s, const unsigned char* in,
  ...
  ZipfliResetHash(ZOPFLI_WINDOW_SIZE, h);
  ...
  for (i = instart; i < inend; i++) {
    ...
    ZipfliFindLongestMatch(s, h, in, i, inend, ZOPFLI_MAX_MATCH, dummysublen,
                           &dist, &leng);
